...
int main(int argc, char *argv[]){
	...
	vecGPU<<<numBlocks, threadsPerBlock>>>(ad, bd, cd, n);
    cudaDeviceSynchronize(); // Wait for the kernel to finish before stopping the clock
...
__global__ void vecGPU(float *ad, float *bd, float *cd, int n) {
    ...
    // Check array bounds to prevent memory access errors
    if (i < n) {
        cd[i] += ad[i] * bd[i];
    }
}
